...
export class UserDevicePermissionRepositoryMongoAdapter implements UserDevicePermissionRepository {
  ...
  constructor(connection: mongoose.Connection) {
    this.permissions = connection.model<UserDevicePermissionSchema>(
      "UserDevicePermission",
      this.userDevicePermissionSchema,
      undefined,
  ...
  remove(id: [Email, DeviceId]): Effect.Effect<void, NotFoundError> {
    const promise = async () =>
      await this.permissions.findOneAndDelete({ email: id[0], deviceId: id[1] })
    return pipe(
      tryPromise(promise),
      orDie,
      flatMap((permission) => {
