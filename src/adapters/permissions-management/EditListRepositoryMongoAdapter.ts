...
export class EditListRepositoryMongoAdapter implements EditListRepository {
  ...
  constructor(connection: mongoose.Connection) {
    this.editLists = connection.model<EditListSchema>("EditList", this.editListSchema, undefined, {
  ...
  update(entity: EditList): Effect.Effect<void, NotFoundError> {
    const promise = async () =>
      await this.editLists.findByIdAndUpdate(entity.id, { users: entity.users }, { new: true })
    return pipe(
      tryPromise(promise),
      orDie,
  ...
  toEntity(editList: EditListSchema): EditList {
    return EditList(
      TaskId(editList._id),
      editList.users.map((user) => Email(user))
    )
  }
}
