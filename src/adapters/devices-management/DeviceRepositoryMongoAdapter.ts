...
export class DeviceRepositoryMongoAdapter
  extends BaseRepositoryMongoAdapter<DeviceId, Device, string, DeviceSchema>
  implements DeviceRepository
{
  ...
  constructor(connection: mongoose.Connection) {
    super(connection)
    this.Device = connection.model("Device", this.deviceSchema, undefined, {
  ...
  private devicePropertyFromSchema(s: DevicePropertySchema): DeviceProperty<unknown> {
    const setterOrTypeConstraints = s.setter
      ? this.deviceActionFromSchema(s.setter)
      : this.typeConstraintFromSchema(s.typeConstraints)
  ...
  private deviceActionFromSchema(s: DeviceActionSchema): DeviceAction<unknown> {
    return DeviceAction(
      DeviceActionId(s.id),
      s.name,
      this.typeConstraintFromSchema(s.inputTypeConstraints),
      s.description
    )
  }
  ...
  protected toDocument(e: Device): Document<unknown, object, DeviceSchema> & DeviceSchema {
    const properties: DevicePropertySchema[] = e.properties.map((p) =>
      this.devicePropertyToSchema(p)
    )
    ...
    return new this.Device({
      _id: e.id,
      name: e.name,
      address: { host: e.address.host, port: e.address.port },
      status: e.status,
  ...
  protected toEntity(s: DeviceSchema): Device {
    ...
    return Device(
      DeviceId(s._id),
      s.name,
      DeviceAddress(s.address.host, s.address.port),
