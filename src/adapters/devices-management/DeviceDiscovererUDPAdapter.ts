...
export class DeviceDiscovererUDPAdapter implements DeviceDiscoverer {
  ...
  constructor(
    ...
    readonly rememberDiscoveriesForSeconds: number,
    { logAnnounces = false }: Options = {}
  ) {
    const socket = dgram.createSocket("udp4", (msg, rinfo) => {
      if (logAnnounces) {
        console.log(`Announce from ${rinfo.address} - ${msg}`)
      }
      ...
      if (isAnnounceMessage(obj)) {
        ...
        if (logAnnounces) {
          console.log("Announce message format was invalid")
        }
      }
      if (logAnnounces) {
        console.log()
