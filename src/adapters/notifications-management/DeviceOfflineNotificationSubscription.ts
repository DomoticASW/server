...
export class DeviceOfflineNotificationSubscriptionRepositoryMongoAdapter
  implements DeviceOfflineNotificationSubscriptionRepository
{
  private deviceOfflineNotificationSubscriptionSchema =
    new mongoose.Schema<DeviceOfflineNotificationSubscriptionSchema>({
      _id: {
  ...
  constructor(connection: mongoose.Connection) {
    this.notifications = connection.model(
      "DeviceOfflineNotificationSubscription",
      this.deviceOfflineNotificationSubscriptionSchema,
      undefined,
  ...
  add(entity: DeviceOfflineNotificationSubscription): Effect<void, DuplicateIdError> {
    return tryPromise({
      try: async () => {
        const notification = new this.notifications({
          _id: { email: entity.email, deviceId: entity.deviceId },
        })
  ...
  find(id: {
    email: Email
    deviceId: DeviceId
  }): Effect<DeviceOfflineNotificationSubscription, NotFoundError, never> {
  ...
  toEntity(
    notification: DeviceOfflineNotificationSubscriptionSchema
  ): DeviceOfflineNotificationSubscription {
    return DeviceOfflineNotificationSubscription(
      Email(notification._id.email),
      DeviceId(notification._id.deviceId)
    )
  }
}
