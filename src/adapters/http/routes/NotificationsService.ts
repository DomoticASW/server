...
export function registerNotificationsServiceRoutes(
  app: express.Express,
  ...
  app.post("/api/notifications", async (req, res) => {
    ...
    const response = await Effect.Do.pipe(
      ...
      Effect.bind("_", ({ token, deviceId }) =>
        service.subscribeForDeviceOfflineNotifications(token, DeviceId(deviceId))
      ),
  ...
  app.delete("/api/notifications/:id", async (req, res) => {
    const response = await Effect.Do.pipe(
      Effect.bind("token", () => deserializeToken(req, usersService)),
      Effect.bind("_", ({ token }) =>
        service.unsubscribeForDeviceOfflineNotifications(token, DeviceId(req.params.id))
      ),
  ...
  app.get("/api/notifications/:id", async (req, res) => {
    const response = await Effect.Do.pipe(
      Effect.bind("token", () => deserializeToken(req, usersService)),
      Effect.bind("result", ({ token }) =>
        service.isSubscribedForDeviceOfflineNotifications(token, DeviceId(req.params.id))
      ),
