...
import {
  AutomationBuilder,
  AutomationBuilderWithDeviceEventTrigger,
  AutomationBuilderWithPeriodtrigger,
  ScriptBuilder,
  TaskBuilder,
} from "../../../domain/scripts-management/ScriptBuilder.js"
...
import {
  ...
  Task,
  TaskId,
} from "../../../domain/scripts-management/Script.js"
import {
  DeviceActionId,
  DeviceId,
  DevicePropertyId,
...
import {
  ...
  isWaitInstruction,
} from "../../../domain/scripts-management/Instruction.js"
import {
  NumberEOperator,
  NumberGEOperator,
  NumberGOperator,
  ...
  isNumberLOperator,
  isStringEOperator,
} from "../../../domain/scripts-management/Operators.js"
import {
  InvalidScriptError,
  ScriptNotFoundError,
...
interface InstructionSchema {
  ...
  instruction:
    | SendNotificationInstructionSchema
    | WaitInstructionSchema
    | StartTaskInstructionSchema
    | DeviceActionInstructionSchema
    | CreateConstantInstructionSchema
    | CreateDevicePropertyConstantInstructionSchema
    | IfInstructionSchema
    | IfElseInstructionSchema
}
...
function isWaitInstructionSchema(o: any): o is WaitInstructionSchema {
  return o && typeof o === "object" && "seconds" in o && typeof o.seconds === "number"
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function isStartTaskInstructionSchema(o: any): o is StartTaskInstructionSchema {
  return o && typeof o === "object" && "taskId" in o && typeof o.taskId === "string"
}
...
function isIfElseInstructionSchema(o: any): o is IfElseInstructionSchema {
  return isIfInstructionSchema(o) && "elseInstructions" in o && Array.isArray(o.elseInstructions)
}
...
function isCondition(o: any): o is ConditionSchema {
  return (
    ...
    typeof o.conditionOperatorType === "string"
  )
}
...
function isInstruction(instruction: any): instruction is InstructionSchema {
  return (
    ...
    (isSendNotificationInstructionSchema(instruction.instruction) ||
...
function serializeInstruction(instruction: Instruction): InstructionSchema {
  ...
  throw new Error(
    "It was not possible to serialize the following instruction into a known type of instruction:\n" +
...
function serializeConditionOperator(operator: ConditionOperator<unknown>): ConditionOperatorType {
  if (isNumberEOperator(operator)) {
    ...
    throw new Error(
      "It was not possible to serialize the following operator into a known type of operator:\n" +
...
export function registerScriptsServiceRoutes(
  app: express.Express,
  ...
  app.post("/api/tasks", async (req, res) => {
    const response = await Effect.Do.pipe(
      ...
      Effect.bind("taskId", ({ token, completeBuilder }) =>
        service.createTask(token, completeBuilder as TaskBuilder)
      ),
  ...
  app.patch("/api/tasks/:id", async (req, res) => {
    const response = await Effect.Do.pipe(
      Effect.bind("token", () => deserializeToken(req, usersService)),
      Effect.bind("completeBuilder", () => completeBuilder(req, "task")),
      Effect.bind("_", ({ token, completeBuilder }) =>
        service.editTask(token, TaskId(req.params.id), completeBuilder as TaskBuilder)
      ),
      Effect.map(() => Response(StatusCodes.OK)),
      Effect.catch("__brand", {
        failure: "ScriptNotFoundError",
  ...
  app.delete("/api/tasks/:id", async (req, res) => {
    const response = await deleteScript(req, usersService, (token) =>
      service.removeTask(token, TaskId(req.params.id))
    )
    sendResponse(res, response)
  })
  ...
  app.get("/api/tasks/:id", async (req, res) => {
    const response = await getScript(req, usersService, (token) =>
      service.findTask(token, TaskId(req.params.id))
    )
    sendResponse(res, response)
  })
  ...
  app.post("/api/automations", async (req, res) => {
    const response = await Effect.Do.pipe(
      ...
      Effect.bind("taskId", ({ token, completeBuilder }) =>
        service.createAutomation(token, completeBuilder as AutomationBuilder)
      ),
      Effect.map(({ taskId }) => Response(StatusCodes.CREATED, { id: taskId })),
      Effect.catch("__brand", {
        failure: "InvalidScriptError",
  ...
  app.patch("/api/automations/:id", async (req, res) => {
    const response = await Effect.Do.pipe(
      ...
      Effect.bind("completeBuilder", () => completeBuilder(req, "automation")),
      Effect.bind("_", ({ token, completeBuilder }) =>
        service.editAutomation(
          token,
          AutomationId(req.params.id),
          completeBuilder as AutomationBuilder
        )
      ),
      Effect.map(() => Response(StatusCodes.OK)),
      Effect.catch("__brand", {
  ...
  app.delete("/api/automations/:id", async (req, res) => {
    const response = await deleteScript(req, usersService, (token) =>
      service.removeAutomation(token, AutomationId(req.params.id))
    )
    sendResponse(res, response)
  })
  ...
  app.get("/api/automations/:id", async (req, res) => {
    const response = await getScript(req, usersService, (token) =>
      service.findAutomation(token, AutomationId(req.params.id))
    )
    sendResponse(res, response)
  })
  ...
  app.get("/api/automations", async (req, res) => {
    const response = await getAllScripts(req, usersService, (token) =>
      service.getAllAutomations(token)
    )
    sendResponse(res, response)
  })
...
function deleteScript(
  ...
  fun: (
    token: Token
  ) => Effect.Effect<void, InvalidTokenError | ScriptNotFoundError | PermissionError>
) {
...
function getScript(
  req: express.Request,
  usersService: UsersService,
  fun: (token: Token) => Effect.Effect<Script<ScriptId>, InvalidTokenError | ScriptNotFoundError>
) {
  return Effect.Do.pipe(
    Effect.bind("token", () => deserializeToken(req, usersService)),
...
function getAllScripts(
  req: express.Request,
  usersService: UsersService,
  fun: (token: Token) => Effect.Effect<Iterable<Script<ScriptId>>, InvalidTokenError>
) {
  return Effect.Do.pipe(
    Effect.bind("token", () => deserializeToken(req, usersService)),
    ...
    Effect.map(({ scripts }) =>
      Response(
        StatusCodes.CREATED,
        Array.from(scripts).map((automation) => serializeScript(automation))
      )
    ),
    handleCommonErrors,
    Effect.runPromise
...
): Effect.Effect<TaskBuilder | AutomationBuilder, BadRequest | InvalidScriptError> {
  return Effect.Do.pipe(
    Effect.bind("scriptNameVal", () =>
      Effect.if(req.body != undefined && "name" in req.body, {
    ...
    Effect.bind("scriptName", ({ scriptNameVal }) => {
      if (typeof scriptNameVal == "string") {
        return Effect.succeed(scriptNameVal)
      } else {
        return Effect.fail(
          BadRequest(`Expected task name of type string but found ${typeof scriptNameVal}`)
        )
      }
    }),
    Effect.bind("trigger", () =>
      Effect.if(type == "automation" && "trigger" in req.body, {
          ...
          Effect.if(isPeriodTrigger(trigger), {
            ...
            onFalse: () =>
              Effect.if(isDeviceEventTrigger(trigger), {
                ...
                onFalse: () =>
                  Effect.fail(
                    BadRequest("Trigger is neither a period trigger nor a device event trigger")
                  ),
              }),
          }),
      })
    ),
    Effect.bind("instructions", () =>
      Effect.if("instructions" in req.body, {
        onTrue: () => Effect.succeed(req.body.instructions),
        onFalse: () => Effect.succeed([]),
      })
    ),
    Effect.bind("instructionsSchemas", ({ instructions }) =>
      createInstructionsSchemas(instructions)
    ),
    Effect.flatMap(({ instructionsSchemas, builderAndRef }) =>
      Effect.if(type == "task", {
        onTrue: () =>
          createBuilderWithInstructions(
            builderAndRef as unknown as [TaskBuilder, NodeRef],
            instructionsSchemas
          ),
        onFalse: () =>
          createBuilderWithInstructions(
            builderAndRef as unknown as [AutomationBuilder, NodeRef],
            instructionsSchemas
          ),
      })
    )
...
function createBuilderWithInstructions<S = Task | Automation>(
  builderAndRef: [ScriptBuilder<S>, NodeRef],
  instructionsSchemas: Array<InstructionSchema>
): Effect.Effect<ScriptBuilder<S>, BadRequest | InvalidScriptError> {
  let taskBuilderAndErr: [ScriptBuilder<S>, BadRequest | InvalidScriptError | undefined] = [
    builderAndRef[0],
    undefined,
  ]
  ...
  for (const instructionSchema of instructionsSchemas) {
    taskBuilderAndErr = createBuilderFromInstructionAndRef(
      taskBuilderAndErr[0]!,
      instructionSchema,
      root,
      constRefs
...
function createBuilderFromInstructionAndRef<S = Task | Automation>(
  builder: ScriptBuilder<S>,
  instructionSchema: InstructionSchema,
  nodeRef: NodeRef,
  constRefs: ConstantRef[]
): [ScriptBuilder<S>, BadRequest | InvalidScriptError | undefined] {
    ...
    case InstructionType.SendNotificationInstruction: {
      if (!isSendNotificationInstructionSchema(instructionSchema.instruction)) {
        return [
          builder,
          BadRequest(
            "It has been tried to create a send notification instruction, but the shape is not the right one"
          ),
        ]
      }
      const instruction =
        instructionSchema.instruction as unknown as SendNotificationInstructionSchema
    ...
    case InstructionType.WaitInstruction: {
      if (!isWaitInstructionSchema(instructionSchema.instruction)) {
        return [
          builder,
          BadRequest(
            "It has been tried to create a wait instruction, but the shape is not the right one"
          ),
        ]
      }
    ...
    case InstructionType.StartTaskInstruction: {
      if (!isStartTaskInstructionSchema(instructionSchema.instruction)) {
        return [
          builder,
          BadRequest(
            "It has been tried to create a start task instruction, but the shape is not the right one"
          ),
        ]
      }
    ...
    case InstructionType.DeviceActionInstruction: {
      if (!isDeviceActionInstructionSchema(instructionSchema.instruction)) {
        return [
          builder,
          BadRequest(
            "It has been tried to create a device action instruction, but the shape is not the right one"
          ),
        ]
      }
      const instruction = instructionSchema.instruction as unknown as DeviceActionInstructionSchema
      builder = builder.addDeviceAction(
        nodeRef,
        DeviceId(instruction.deviceId),
        DeviceActionId(instruction.deviceActionId),
        instruction.input
    ...
    case InstructionType.CreateConstantInstruction: {
      if (!isCreateConstantInstructionSchema(instructionSchema.instruction)) {
        return [
          builder,
          BadRequest(
            "It has been tried to create a create constant instruction, but the shape is not the right one"
          ),
        ]
      }
      const instruction =
        instructionSchema.instruction as unknown as CreateConstantInstructionSchema
      const builderAndRef = builder.addCreateConstant(
        nodeRef,
        instruction.name,
        instruction.type,
        instruction.value
    ...
    case InstructionType.CreateDevicePropertyConstantInstruction: {
      if (!isCreateDevicePropertyConstantInstructionSchema(instructionSchema.instruction)) {
        return [
          builder,
          BadRequest(
            "It has been tried to create a create device property constant instruction, but the shape is not the right one"
          ),
        ]
      }
      const instruction =
        instructionSchema.instruction as unknown as CreateDevicePropertyConstantInstructionSchema
      const builderAndRef = builder.addCreateDevicePropertyConstant(
        nodeRef,
        instruction.name,
        instruction.type,
        DeviceId(instruction.deviceId),
    ...
    case InstructionType.IfInstruction: {
      if (!isIfInstructionSchema(instructionSchema.instruction)) {
        return [
          builder,
          BadRequest(
            "It has been tried to create an if instruction, but the shape is not the right one"
          ),
        ]
      }
      const ifInstructionSchema = instructionSchema.instruction as unknown as IfInstructionSchema
      const left = constRefs.find(
        (ref) => ref.constantInstruction.name == ifInstructionSchema.condition.leftConstantName
      )
      const right = constRefs.find(
        (ref) => ref.constantInstruction.name == ifInstructionSchema.condition.rightConstantName
      )
      if (!left || !right) {
        return [
          builder,
          InvalidScriptError(
            "One or two of the two constants " +
              ifInstructionSchema.condition.leftConstantName +
              " and " +
              ifInstructionSchema.condition.rightConstantName +
              " are not declared before the If"
          ),
        ]
      }
      const builderAndRef = builder.addIf(
        nodeRef,
        left,
        right,
        deserializeConditionOperator(ifInstructionSchema.condition.conditionOperatorType),
        ifInstructionSchema.condition.negate
      )
      builder = builderAndRef[0]
      const then = builderAndRef[1]
      const thenBuilderAndErr = putBranchInstructionsToBuilder(
        ifInstructionSchema.thenInstructions,
        builder,
        then,
        constRefs
      )
    ...
    case InstructionType.IfElseInstruction: {
      if (!isIfElseInstructionSchema(instructionSchema.instruction)) {
        return [
          builder,
          BadRequest(
            "It has been tried to create an if else instruction, but the shape is not the right one"
          ),
        ]
      }
      const ifInstructionSchema =
        instructionSchema.instruction as unknown as IfElseInstructionSchema
      const left = constRefs.find(
        (ref) => ref.constantInstruction.name == ifInstructionSchema.condition.leftConstantName
      )
      const right = constRefs.find(
      ...
      const builderAndRef = builder.addIfElse(
        nodeRef,
        left,
        right,
        deserializeConditionOperator(ifInstructionSchema.condition.conditionOperatorType),
      ...
      const elseBuilderAndErr = putBranchInstructionsToBuilder(
        ifInstructionSchema.elseInstructions,
        builder,
        elseRef,
        constRefs
      )
...
function putBranchInstructionsToBuilder<S = Task | Automation>(
  instructions: InstructionSchema[],
  builder: ScriptBuilder<S>,
  nodeRef: NodeRef,
  constRefs: ConstantRef[]
  ...
  for (const instruction of instructions) {
    const newBuilderAndErr = createBuilderFromInstructionAndRef(
      newBuilder,
      instruction,
      nodeRef,
      constRefs
