...
export function registerUsersServiceRoutes(app: express.Application, service: UsersService) {
    ...
    const response = await Effect.Do.pipe(
      Effect.bind("nicknameVal", () => {
        if (req.body && "nickname" in req.body) {
          return Effect.succeed(req.body["nickname"])
        } else {
          return Effect.fail(BadRequest(`Expected body format is: {nickname: ???}`))
        }
      }),
      Effect.bind("nicknameString", ({ nicknameVal }) => {
        if (typeof nicknameVal == "string") {
          return Effect.succeed(nicknameVal)
        } else {
          return Effect.fail(
            BadRequest(`Expected nickname of type string but found ${typeof nicknameVal}`)
          )
        }
      }),
      Effect.bind("emailVal", () => {
        if (req.body && "email" in req.body) {
          return Effect.succeed(req.body["email"])
        } else {
          return Effect.fail(BadRequest(`Expected body format is: {email: ???}`))
        }
      }),
      Effect.bind("emailString", ({ emailVal }) => {
        if (typeof emailVal == "string") {
          return Effect.succeed(emailVal)
        } else {
          return Effect.fail(
            BadRequest(`Expected email of type string but found ${typeof emailVal}`)
          )
        }
      }),
      Effect.bind("passwordVal", () => {
        if (req.body && "password" in req.body) {
          return Effect.succeed(req.body["password"])
        } else {
          return Effect.fail(BadRequest(`Expected body format is: {password: ???}`))
        }
      }),
      Effect.bind("passwordString", ({ passwordVal }) => {
        if (typeof passwordVal == "string") {
          return Effect.succeed(passwordVal)
        } else {
          return Effect.fail(
            BadRequest(`Expected password of type string but found ${typeof passwordVal}`)
          )
        }
      }),
      ...
      Effect.bind("password", ({ passwordString }) =>
        Effect.succeed(ClearTextPassword(passwordString))
      ),
      Effect.bind("_", ({ nickname, email, password }) =>
        service.publishRegistrationRequest(nickname, email, password)
      ),
  ...
  app.post("/api/registrationRequests/:id/approve", async (req, res) => {
    const response = await Effect.Do.pipe(
      Effect.bind("token", () => deserializeToken(req, service)),
      Effect.bind("_", ({ token }) =>
        service.approveRegistrationRequest(token, Email(req.params.id))
      ),
  ...
  app.post("/api/registrationRequests/:id/reject", async (req, res) => {
    const response = await Effect.Do.pipe(
      Effect.bind("token", () => deserializeToken(req, service)),
      Effect.bind("_", ({ token }) =>
        service.rejectRegistrationRequest(token, Email(req.params.id))
      ),
      Effect.map(() => Response(StatusCodes.OK)),
      Effect.catch("__brand", {
        failure: "RegistrationRequestNotFoundError",
  ...
  app.patch("/api/users", async (req, res) => {
    const response = await Effect.Do.pipe(
      ...
      Effect.bind("_", ({ token, nickname, password }) =>
        service.updateUserData(token, nickname, password)
      ),
      Effect.map(() => Response(StatusCodes.OK)),
      Effect.catch("__brand", {
        failure: "UserNotFoundError",
    ...
    const response = await Effect.Do.pipe(
      ...
      Effect.bind("passwordString", ({ passwordVal }) => {
        if (typeof passwordVal == "string") {
          ...
          return Effect.fail(
            BadRequest(`Expected ${key} of type string but found ${typeof passwordVal}`)
          )
        }
      }),
      Effect.bind("token", ({ emailString, passwordString }) =>
        service.login(Email(emailString), ClearTextPassword(passwordString))
      ),
