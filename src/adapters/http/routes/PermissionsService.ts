...
export function registerPermissionsServiceRoutes(
  app: express.Express,
  ...
  app.get("/api/permissions/user-device/:id", async (req, res) => {
    ...
    const response = await Effect.Do.pipe(
      ...
      Effect.bind("userDevicePermission", ({ token, email }) =>
        service.findUserDevicePermission(token, Email(email), DeviceId(req.params.id))
      ),
  ...
  app.get("/api/permissions/user-device-all/:id", async (req, res) => {
    const response = await Effect.Do.pipe(
      Effect.bind("token", () => deserializeToken(req, usersService)),
      Effect.bind("userDevicePermissions", ({ token }) =>
        service.findAllUserDevicePermissionsOfAnUser(token, Email(req.params.id))
      ),
      Effect.map(({ userDevicePermissions }) =>
        Response(StatusCodes.OK, Array.from(userDevicePermissions))
      ),
      Effect.catch("__brand", {
  ...
  app.get("/api/permissions/user-device", async (req, res) => {
    const response = await Effect.Do.pipe(
      Effect.bind("token", () => deserializeToken(req, usersService)),
      Effect.bind("userDevicePermissions", ({ token }) =>
        service.getAllUserDevicePermissions(token)
      ),
      Effect.map(({ userDevicePermissions }) =>
        Response(StatusCodes.OK, Array.from(userDevicePermissions))
      ),
  ...
  app.post("/api/permissions/user-device/:id", async (req, res) => {
    ...
    const response = await Effect.Do.pipe(
      ...
      Effect.bind("_", ({ token, email }) =>
        service.addUserDevicePermission(token, Email(email), DeviceId(req.params.id))
      ),
  ...
  app.delete("/api/permissions/user-device/:id", async (req, res) => {
    ...
    const response = await Effect.Do.pipe(
      ...
      Effect.bind("_", ({ token, email }) =>
        service.removeUserDevicePermission(token, Email(email), DeviceId(req.params.id))
      ),
      Effect.map(() => Response(StatusCodes.OK)),
      Effect.catch("__brand", {
        failure: "UserNotFoundError",
  ...
  app.get("/api/permissions/canExecute/device/:id", async (req, res) => {
    const response = await Effect.Do.pipe(
      Effect.bind("token", () => deserializeToken(req, usersService)),
      Effect.bind("_", ({ token }) =>
        service.canExecuteActionOnDevice(token, DeviceId(req.params.id))
      ),
      Effect.map(() => Response(StatusCodes.OK)),
      handleCommonErrors,
      Effect.runPromise
  ...
  app.patch("/api/permissions/blacklist/:id", async (req, res) => {
    ...
    const response = await Effect.Do.pipe(
      ...
      Effect.bind("_", ({ token, email }) =>
        service.addToBlacklist(token, Email(email), TaskId(req.params.id))
      ),
      Effect.map(() => Response(StatusCodes.OK)),
      Effect.catch("__brand", {
        failure: "UserNotFoundError",
  ...
  app.patch("/api/permissions/whitelist/:id", async (req, res) => {
    ...
    const response = await Effect.Do.pipe(
      ...
      Effect.bind("_", ({ token, email }) =>
        service.addToWhitelist(token, Email(email), TaskId(req.params.id))
      ),
      Effect.map(() => Response(StatusCodes.OK)),
      Effect.catch("__brand", {
        failure: "UserNotFoundError",
  ...
  app.patch("/api/permissions/editlist/:id", async (req, res) => {
    ...
    const response = await Effect.Do.pipe(
      ...
      Effect.bind("_", ({ token, email }) =>
        service.addToEditlist(token, Email(email), TaskId(req.params.id))
      ),
      Effect.map(() => Response(StatusCodes.OK)),
      Effect.catch("__brand", {
        failure: "UserNotFoundError",
  ...
  app.delete("/api/permissions/blacklist/:id", async (req, res) => {
    ...
    const response = await Effect.Do.pipe(
      ...
      Effect.bind("_", ({ token, email }) =>
        service.removeFromBlacklist(token, Email(email), TaskId(req.params.id))
      ),
      Effect.map(() => Response(StatusCodes.OK)),
      Effect.catch("__brand", {
        failure: "UserNotFoundError",
  ...
  app.delete("/api/permissions/whitelist/:id", async (req, res) => {
    ...
    const response = await Effect.Do.pipe(
      ...
      Effect.bind("_", ({ token, email }) =>
        service.removeFromWhitelist(token, Email(email), TaskId(req.params.id))
      ),
      Effect.map(() => Response(StatusCodes.OK)),
      Effect.catch("__brand", {
        failure: "UserNotFoundError",
  ...
  app.delete("/api/permissions/editlist/:id", async (req, res) => {
    ...
    const response = await Effect.Do.pipe(
      ...
      Effect.bind("_", ({ token, email }) =>
        service.removeFromEditlist(token, Email(email), TaskId(req.params.id))
      ),
      Effect.map(() => Response(StatusCodes.OK)),
      Effect.catch("__brand", {
        failure: "UserNotFoundError",
