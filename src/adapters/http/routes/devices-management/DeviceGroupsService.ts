...
export function registerDeviceGroupsServiceRoutes(
  app: express.Express,
  ...
  app.get("/api/deviceGroups/:id", async (req, res) => {
    const response = await Effect.Do.pipe(
      Effect.bind("token", () => deserializeToken(req, usersService)),
      Effect.bind("deviceGroup", ({ token }) =>
        service.findGroup(token, DeviceGroupId(req.params.id))
      ),
      Effect.bind("deviceGroupDTO", ({ deviceGroup }) =>
        DeviceGroupDTO(deviceGroup, devicesService)
      ),
  ...
  app.post("/api/deviceGroups/:id", async (req, res) => {
    ...
    const response = await Effect.Do.pipe(
      Effect.bind("token", () => deserializeToken(req, usersService)),
      Effect.bind("_", () =>
        Effect.if(req.body != undefined && bodyProperty in req.body, {
          onTrue: () => Effect.succeed(null),
          onFalse: () =>
            Effect.fail(BadRequest(`Missing ${bodyProperty} property in request body`)),
        })
      ),
      Effect.bind("__", ({ token }) =>
        service.renameGroup(token, DeviceGroupId(req.params.id), req.body[bodyProperty])
      ),
  ...
  app.get("/api/deviceGroups", async (req, res) => {
    const response = await Effect.Do.pipe(
      ...
      Effect.bind("deviceGroupDTOs", ({ deviceGroups }) =>
        Effect.all(Array.from(deviceGroups).map((dg) => DeviceGroupDTO(dg, devicesService)))
      ),
  ...
  app.post("/api/deviceGroups/:id/device", async (req, res) => {
    ...
    const response = await Effect.Do.pipe(
      ...
      Effect.bind("__", ({ token }) =>
        service.addDeviceToGroup(
          token,
          DeviceId(req.body[bodyProperty]),
