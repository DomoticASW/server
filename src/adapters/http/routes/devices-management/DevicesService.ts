...
export function registerDevicesServiceRoutes(
  app: express.Express,
  server: SocketIOServer,
  service: DevicesService,
  actionsService: DeviceActionsService,
  ...
  app.post("/api/devices", async (req, res) => {
    ...
    const response = await Effect.Do.pipe(
      ...
      Effect.bind("deviceAddress", () => {
        if (req.body && key in req.body && isDeviceAddress(req.body[key])) {
          return Effect.succeed(req.body[key])
        } else {
          return Effect.fail(
            BadRequest(`Expected body format is: {${key}: {host: ..., port: ...}}`)
          )
        }
      }),
    ...
    const response = await Effect.Do.pipe(
      ...
      Effect.bind("name", ({ nameVal }) => {
        if (typeof nameVal == "string") {
          return Effect.succeed(nameVal)
        } else {
          return Effect.fail(
            BadRequest(`Expected ${key} of type string but found ${typeof nameVal}`)
          )
        }
      }),
  ...
  app.get("/api/devices", async (req, res) => {
    const response = await Effect.Do.pipe(
      ...
      Effect.map(({ devices }) =>
        Response(
          StatusCodes.OK,
          Array.from(devices).map((d) => DeviceDTO(d))
        )
      ),
      handleCommonErrors,
      Effect.runPromise
  ...
  app.patch("/api/devices/:id/properties/:propertyId", async (req, res) => {
    ...
    const response = await Effect.Do.pipe(
      ...
      Effect.bind("_", ({ value }) =>
        service.updateDeviceProperty(
          DeviceId(req.params.id),
          DevicePropertyId(req.params.propertyId),
          value
        )
      ),
      Effect.map(() => Response(StatusCodes.OK)),
    ...
    const response = await Effect.Do.pipe(
      Effect.bind("updates", () => {
        if (isUpdateDevicePropertiesBody(req.body)) {
          ...
          return Effect.fail(
            BadRequest(`Expected body format is: [{propertyId: ..., value: ...}, ...]`)
          )
        }
      }),
      Effect.bind("_", ({ updates }) => {
        const updatesMap = new Map(
          updates.map((item) => [DevicePropertyId(item.propertyId), item.value])
        )
  ...
  app.post("/api/devices/:id/actions/:actionId/execute", async (req, res) => {
    ...
    const response = await Effect.Do.pipe(
      ...
      Effect.bind("_", ({ token, input }) =>
        actionsService.executeAction(
          token,
          DeviceId(req.params.id),
          DeviceActionId(req.params.actionId),
          input
        )
      ),
      Effect.map(() => Response(StatusCodes.OK)),
