...
export function BadRequestError(message?: string, cause?: string): BadRequestError {
  return {
    message: message ?? "The request was malformed",
    cause: cause,
    __brand: "BadRequestError",
  }
}
...
export class HTTPServerAdapter {
  ...
  constructor(
    ...
    app.use((req, res, next) => {
      express.json()(req, res, (err) => {
        if (err) {
          return res
            .status(400)
            .send(BadRequestError("The received body was not valid JSON", err.message))
        } else {
          next()
        }
      })
    ...
    app.use(async (req: Request, _res: Response, next: NextFunction) => {
      ...
      if (logRequestUrls || logRequestBodies) {
        console.log()
      }
...
export function registerNotificationsServiceProtocol(
