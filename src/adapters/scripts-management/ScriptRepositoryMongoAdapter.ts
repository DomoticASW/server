...
import {
  ...
  WaitInstruction,
} from "../../domain/scripts-management/InstructionImpl.js"
import {
  BooleanEOperator,
  ColorEOperator,
  isBooleanEOperator,
...
export class ScriptRepositoryMongoAdapter
  extends BaseRepositoryMongoAdapter<ScriptId, Script<ScriptId>, string, ScriptSchema>
  implements ScriptRepository
{
  ...
  constructor(connection: mongoose.Connection) {
    super(connection)
    this.Script = connection.model("Script", this.scriptSchema, undefined, {
  ...
  protected toDocument(
    e: Script<ScriptId>
  ): Document<unknown, object, ScriptSchema> & ScriptSchema {
    ...
    return new this.Script({
      ...
      name: e.name,
      scriptType: scriptType,
      instructions: this.serializeInstructions(e.instructions),
  ...
  private serializeInstructions(instructions: Instruction[]): InstructionSchema[] {
    return instructions.map((i) => {
      ...
      if (isSendNotificationInstruction(i)) {
        ...
        instruction = {
          then: this.serializeInstructions(i.then),
          condition: this.serializeCondition(i.condition),
        }
        type = InstructionType.IfInstruction
      } else if (isIfElseInstruction(i)) {
        instruction = {
          then: this.serializeInstructions(i.then),
          else: this.serializeInstructions(i.else),
          condition: this.serializeCondition(i.condition),
        }
        ...
        instruction = {
          ...
          type: i.type,
          deviceId: i.deviceId,
          devicePropertyId: i.devicePropertyId,
        }
  ...
  private deserializeInstructions(
    instructions: InstructionSchema[],
    superInstructions: InstructionSchema[]
  ): Instruction[] {
    return instructions.map((instruction) => {
      switch (instruction.type) {
        ...
        case InstructionType.DeviceActionInstruction: {
          ...
          return DeviceActionInstruction(
            DeviceId(i.deviceId),
            DeviceActionId(i.deviceActionId),
            i.input
          )
        }
        ...
        case InstructionType.IfElseInstruction: {
          ...
          const [leftConstantInstructionSchema, leftConstantInstructionSchemaType] =
            this.findConstantInstructionSchemaByName(
              superInstructions.concat(instructions),
              i.condition.leftConstantName
            )!
          const [rightConstantInstructionSchema, rightConstantInstructionSchemaType] =
            this.findConstantInstructionSchemaByName(
              superInstructions.concat(instructions),
              i.condition.rightConstantName
            )!
          // Deserializing constant instructions
          const leftConstantInstruction = this.deserializeConstantInstruction(
            leftConstantInstructionSchema,
            leftConstantInstructionSchemaType
          )
          const rightConstantInstruction = this.deserializeConstantInstruction(
            rightConstantInstructionSchema,
            rightConstantInstructionSchemaType
          )
          const conditionOperator = this.deserializeConditionOperator(
            i.condition.conditionOperatorType
          )

          if (instruction.type == InstructionType.IfInstruction) {
            return IfInstruction(
              this.deserializeInstructions(i.then, superInstructions.concat(instructions)),
              Condition(
                leftConstantInstruction,
                rightConstantInstruction,
                conditionOperator,
                i.condition.negate
              )
            )
            ...
            return IfElseInstruction(
              this.deserializeInstructions(i.then, superInstructions.concat(instructions)),
              this.deserializeInstructions(
                (i as IfElseInstructionSchema).else,
                superInstructions.concat(instructions)
              ),
              Condition(
                leftConstantInstruction,
  ...
  private serializeCondition(condition: Condition<unknown>): ConditionSchema {
    return {
      ...
      conditionOperatorType: this.serializeConditionOperator(condition.operator),
  ...
  private findConstantInstructionSchemaByName(
    instructions: InstructionSchema[],
    name: string
  ):
    | [
        ConstantInstructionSchema,
        (
          | InstructionType.CreateConstantInstruction
          | InstructionType.CreateDevicePropertyConstantInstruction
        ),
      ]
    | undefined {
    const instr = instructions.find(
      (i) =>
        (i.type == InstructionType.CreateConstantInstruction ||
          i.type == InstructionType.CreateDevicePropertyConstantInstruction) &&
        (i.instruction as ConstantInstructionSchema).name == name
    )
    if (instr) {
      return [
        instr.instruction as ConstantInstructionSchema,
        instr.type as
          | InstructionType.CreateConstantInstruction
          | InstructionType.CreateDevicePropertyConstantInstruction,
      ]
  ...
  private deserializeConstantInstruction(
    instruction: ConstantInstructionSchema,
    type:
      | InstructionType.CreateConstantInstruction
      | InstructionType.CreateDevicePropertyConstantInstruction
  ): ConstantInstruction<unknown> {
    switch (type) {
      case InstructionType.CreateConstantInstruction: {
      ...
      case InstructionType.CreateDevicePropertyConstantInstruction: {
        ...
        return CreateDevicePropertyConstantInstruction(
          i.name,
          i.type,
          DeviceId(i.deviceId),
          DevicePropertyId(i.devicePropertyId)
