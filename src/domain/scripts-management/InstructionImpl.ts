...
export function SendNotificationInstruction(email: Email, message: string): SendNotificationInstruction {
  return {
    ...
    execute(env) {
      return pipe(
        env.notificationsService.sendNotification(email, message),
        map(() => env),
...
export function StartTaskInstruction(taskId: TaskId): StartTaskInstruction {
  return {
    ...
    execute(env) {
      return pipe(
        env.taskToken
          ? env.permissionsService.canExecuteTask(env.taskToken, taskId)
          : Effect.void,
        flatMap(() =>
          pipe(
            env.scriptsService.findTaskUnsafe(taskId),
            flatMap(task =>
              pipe(
                task.execute(env.notificationsService, env.scriptsService, env.permissionsService, env.devicesService, env.taskToken),
                andThen(() => succeed(env))
              )
            ),
          )
...
export function DeviceActionInstruction(deviceId: DeviceId, deviceActionId: DeviceActionId, input: unknown): DeviceActionInstruction {
  return {
    ...
    execute(env) {
      return pipe(
        env.devicesService.executeAutomationAction(deviceId, deviceActionId, input),
        map(() => env),
        mapError(error => ScriptError(error.message + ", " + error.cause))
      )
    },
...
export function CreateDevicePropertyConstantInstruction<T>(name: string, type: Type, deviceId: DeviceId, devicePropertyId: DevicePropertyId): CreateDevicePropertyConstantInstruction<T> {
  return {
    ...
    execute(env) {
      return pipe(
        env.devicesService.findUnsafe(deviceId),
        flatMap(device => {
