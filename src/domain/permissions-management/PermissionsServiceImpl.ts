...
import {
  EditListNotFoundError,
  InvalidOperationError,
  PermissionError,
  TaskListsNotFoundError,
  UserDevicePermissionNotFoundError,
} from "../../ports/permissions-management/Errors.js"
...
import {
  ...
  UnauthorizedError,
  UserNotFoundError,
} from "../../ports/users-management/Errors.js"
...
} from "../scripts-management/Instruction.js"
...
export class PermissionsServiceImpl implements PermissionsService {
  ...
  constructor(
    userDevicePermissionRepo: UserDevicePermissionRepository,
    taskListsRepo: TaskListsRepository,
    editListRepo: EditListRepository,
  ...
  findUserDevicePermission(
    token: Token,
    email: Email,
    deviceId: DeviceId
  ): Effect.Effect<UserDevicePermission, TokenError | UserDevicePermissionNotFoundError> {
    return pipe(
  ...
  findAllUserDevicePermissionsOfAnUser(
    token: Token,
    email: Email
  ): Effect.Effect<Iterable<UserDevicePermission>, UserNotFoundError | TokenError> {
    return pipe(
  ...
  getAllUserDevicePermissions(
    token: Token
  ): Effect.Effect<Iterable<UserDevicePermission>, TokenError> {
    return pipe(
      Effect.if(token.role == Role.Admin, {
        onTrue: () => this.usersService.verifyToken(token),
        onFalse: () => Effect.fail(UnauthorizedError()),
  ...
  addUserDevicePermission(
    ...
    email: Email,
    deviceId: DeviceId
  ): Effect.Effect<void, UserNotFoundError | DeviceNotFoundError | TokenError> {
    return pipe(
      Effect.if(token.role == Role.Admin, {
        onTrue: () => this.usersService.verifyToken(token),
        onFalse: () => Effect.fail(UnauthorizedError()),
      ...
      Effect.flatMap(() =>
        this.userDevicePermissionRepo.add(UserDevicePermission(email, deviceId))
      ),
      Effect.catch("__brand", {
  ...
  removeUserDevicePermissionUnsafe(
    token: Token,
    email: Email,
    deviceId: DeviceId
  ): Effect.Effect<void, UserNotFoundError | DeviceNotFoundError | TokenError> {
  ...
  canExecuteActionOnDevice(
    token: Token,
    deviceId: DeviceId
  ): Effect.Effect<void, PermissionError | InvalidTokenError> {
    return pipe(
  ...
  canExecuteTask(
    token: Token,
    taskId: TaskId,
    visitedTaskIds: TaskId[] = [taskId]
  ): Effect.Effect<void, PermissionError | InvalidTokenError | ScriptNotFoundError> {
    return pipe(
      this.usersService.verifyToken(token),
      Effect.flatMap(() =>
        pipe(
          ...
          Effect.flatMap(
            (
              ...
              Task | undefined,
              PermissionError | InvalidTokenError | ScriptNotFoundError
            > => {
              ...
              return pipe(this.scriptService.findTask(token, taskId))
            }
          ),
          Effect.catch("__brand", {
            failure: "NotFoundError",
            onFailure: () => pipe(this.scriptService.findTask(token, taskId)),
          })
        )
      ),
      Effect.flatMap((task) => {
        ...
        const flattenedInstructions = task.instructions.flatMap((i) => this.flatInstructions(i))
        ...
        const tasksChecks = flattenedInstructions
          ...
          .map((instruction) =>
            this.canExecuteTask(
              token,
              instruction.taskId,
              visitedTaskIds.concat([instruction.taskId])
            )
          )
  ...
  private flatInstructions(instruction: Instruction): Instruction[] {
    return isIfElseInstruction(instruction)
      ? instruction.then.concat(instruction.else).flatMap((i) => this.flatInstructions(i))
      : isIfInstruction(instruction)
        ? instruction.then.flatMap((i) => this.flatInstructions(i))
        : [instruction]
  }
  ...
  findEditList(
    token: Token,
    scriptId: ScriptId
  ): Effect.Effect<EditList, TokenError | EditListNotFoundError> {
    return pipe(
      Effect.if(token.role === Role.Admin, {
        onTrue: () => this.usersService.verifyToken(token),
        onFalse: () => Effect.fail(UnauthorizedError()),
  ...
  addToEditlist(
    ...
    void,
    TokenError | UserNotFoundError | ScriptNotFoundError | EditListNotFoundError
  > {
    return pipe(
      Effect.if(token.role === Role.Admin, {
        onTrue: () => this.usersService.verifyToken(token),
  ...
  addToEditlistUnsafe(
    email: Email,
    scriptId: ScriptId
  ): Effect.Effect<void, UserNotFoundError | ScriptNotFoundError | EditListNotFoundError> {
    return pipe(
      this.usersService.getUserDataUnsafe(email),
      ...
      Effect.catch("__brand", {
        ...
        onFailure: () =>
          Effect.fail(
            EditListNotFoundError(
              "Something went wrong while creating the editlist of the newly created script. Maybe a concurrency issue."
            )
          ),
      })
    )
  ...
  findTaskLists(
    token: Token,
    taskId: TaskId
  ): Effect.Effect<TaskLists, TokenError | TaskListsNotFoundError> {
    return pipe(
      Effect.if(token.role == Role.Admin, {
        onTrue: () => this.usersService.verifyToken(token),
        onFalse: () => Effect.fail(UnauthorizedError()),
  ...
  addToWhitelist(
    ...
    void,
    TokenError | UserNotFoundError | ScriptNotFoundError | InvalidOperationError
  > {
    return pipe(
      Effect.if(token.role == Role.Admin, {
        onTrue: () => this.usersService.verifyToken(token),
  ...
  removeFromWhitelist(
    ...
    email: Email,
    taskId: TaskId
  ): Effect.Effect<void, TokenError | UserNotFoundError | ScriptNotFoundError> {
    return pipe(
      Effect.if(token.role == Role.Admin, {
        onTrue: () => this.usersService.verifyToken(token),
        onFalse: () => Effect.fail(UnauthorizedError()),
...
function isDeviceActionInstruction(
  instruction: Instruction
): instruction is DeviceActionInstruction {
  return (
    instruction &&
