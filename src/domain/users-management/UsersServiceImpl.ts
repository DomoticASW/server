...
export class UsersServiceImpl implements UsersService {
  ...
  getAllRegistrationRequests(
    token: Token
  ): Effect<Iterable<RegistrationRequest>, InvalidTokenError> {
    return pipe(
  ...
  approveRegistrationRequest(
    token: Token,
    email: Email
  ): Effect<void, EmailAlreadyInUseError | RegistrationRequestNotFoundError | TokenError> {
    return pipe(
        ...
        switch (e.__brand) {
          ...
          case "DuplicateIdError":
            return EmailAlreadyInUseError(
              "It was not possible to approve this request as the email is already used by a user"
            )
          default:
            return e
        }
  ...
  rejectRegistrationRequest(
    token: Token,
    email: Email
  ): Effect<void, RegistrationRequestNotFoundError | TokenError> {
    return pipe(
      Eff.if(token.role == Role.Admin, {
        onTrue: () => this.verifyToken(token),
        onFalse: () => Eff.fail(UnauthorizedError()),
    ...
    return pipe(
      ...
      Eff.flatMap(() => {
        if (token.role == Role.Admin && token.userEmail == email) {
          ...
          return Eff.fail(
            UnauthorizedError(
              "You cannot remove other users from the system, only the admin is allowed"
            )
          )
        }
        ...
        onSuccess: (user) => {
          return pipe(
            ...
            Eff.flatMap((result) => {
              ...
              const source = jwt.sign({ userEmail: user.email, role: user.role }, this.secret, {
                expiresIn: "1h",
              })
  ...
  verifyToken(token: Token): Effect<void, InvalidTokenError> {
    return pipe(
      Eff.try({
        try: () =>
          jwt.verify(token.source, this.secret, {
