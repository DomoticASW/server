...
export class DeviceGroupsServiceImpl implements DeviceGroupsService {
  ...
  constructor(
    repo: Repository<DeviceGroupId, DeviceGroup>,
  ...
  addGroup(
    token: Token,
    name: string
  ): Effect.Effect<DeviceGroupId, DeviceGroupNameAlreadyInUseError | TokenError> {
  ...
  ): Effect.Effect<void, DeviceGroupNotFoundError | TokenError> {
    return pipe(
      Effect.if(token.role == Role.Admin, {
        onTrue: () => this.usersService.verifyToken(token),
        onFalse: () => Effect.fail(UnauthorizedError()),
  ...
  renameGroup(
    ...
    groupId: DeviceGroupId,
    name: string
  ): Effect.Effect<void, DeviceGroupNotFoundError | DeviceGroupNameAlreadyInUseError | TokenError> {
  ...
  findGroup(
    token: Token,
    groupId: DeviceGroupId
  ): Effect.Effect<DeviceGroup, DeviceGroupNotFoundError | InvalidTokenError> {
    return pipe(
  ...
  addDeviceToGroup(
    ...
    deviceId: DeviceId,
    groupId: DeviceGroupId
  ): Effect.Effect<void, DeviceNotFoundError | DeviceGroupNotFoundError | TokenError> {
    return Effect.Do.pipe(
      Effect.bind("", () =>
        Effect.if(token.role == Role.Admin, {
          onTrue: () => this.usersService.verifyToken(token),
      ...
      Effect.catch("__brand", {
        ...
        onFailure: (e) =>
          Effect.dieMessage("Unexpected error while adding a device to a device group: " + e),
      }),
      Effect.mapError((e) => {
        switch (e.__brand) {
