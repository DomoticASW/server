  ...
  validate(value: string): Effect.Effect<void, InvalidValueError> {
    return Effect.if(this.values.has(value), {
      ...
      onFalse: () =>
        Effect.fail(
          InvalidValueError(
            `Value ${value} is not valid, valid values are:\n${Array.from(this.values).join(", ")}`
          )
        ),
    })
  }
  ...
  validate(value: number): Effect.Effect<void, InvalidValueError> {
    return Effect.flatMap(
      Effect.if(Number.isInteger(value), {
        onTrue: () => Effect.succeed(null),
        onFalse: () =>
          Effect.fail(InvalidValueError(`Value ${value} is not valid as it's not an integer`)),
      }),
      () =>
        Effect.if(value >= this.min && value <= this.max, {
          ...
          onFalse: () =>
            Effect.fail(
              InvalidValueError(
                `Value ${value} is not valid since it's not between ${this.min} and ${this.max}`
              )
            ),
        })
    )
    ...
    switch (this.type) {
      case Type.IntType:
        return Number.isInteger(value)
          ...
          : Effect.fail(appropriateError("int"))
      case Type.DoubleType:
        return typeof value == "number"
          ? Effect.succeed(undefined)
          : Effect.fail(appropriateError("double"))
      case Type.BooleanType:
        return typeof value == "boolean"
          ? Effect.succeed(undefined)
          : Effect.fail(appropriateError("boolean"))
      case Type.StringType:
        return typeof value == "string"
          ? Effect.succeed(undefined)
          : Effect.fail(appropriateError("string"))
      case Type.ColorType:
