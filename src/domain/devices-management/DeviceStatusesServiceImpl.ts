...
import {
  DeviceStatusChangesSubscriber,
  DeviceStatusesService,
} from "../../ports/devices-management/DeviceStatusesService.js"
...
export class DeviceStatusesServiceImpl implements DeviceStatusesService {
  ...
  constructor(
    pollingRateMillis: number,
    devicesService: DevicesService,
    protocol: DeviceCommunicationProtocol,
    { logDeviceStatusChanges = false }: Options = {}
  ) {
        ...
        (r) =>
          Effect.Do.pipe(
            ...
            Effect.flatMap(({ device }) =>
              Effect.if(device.status == r.status, {
                onTrue: () => Effect.void,
                onFalse: () => {
                  if (this.logDeviceStatusChanges) {
                    console.log(`Devide ${device.name} is now ${r.status}\n`)
                  }
                  return pipe(
                    this.subscribers,
                    Effect.forEach((s) => s.deviceStatusChanged(r.deviceId, r.status), {
                      concurrency: "unbounded",
                    }),
                    Effect.flatMap(() =>
                      this.devicesService.setDeviceStatusUnsafe(r.deviceId, r.status)
                    ),
