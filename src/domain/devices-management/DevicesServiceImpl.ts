...
import {
  DevicePropertyUpdatesSubscriber,
  DevicesService,
} from "../../ports/devices-management/DevicesService.js"
import {
...
export class DevicesServiceImpl implements DevicesService {
  ...
  add(
    ...
    DeviceId,
    DeviceAlreadyRegisteredError | DeviceUnreachableError | CommunicationError | TokenError
  > {
  ...
  rename(
    ...
    deviceId: DeviceId,
    name: string
  ): Effect.Effect<void, DeviceNotFoundError | TokenError> {
    return Effect.Do.pipe(
      Effect.bind("_", () =>
        Effect.if(token.role == Role.Admin, {
          onTrue: () => this.usersService.verifyToken(token),
  ...
  find(
    token: Token,
    deviceId: DeviceId
  ): Effect.Effect<Device, DeviceNotFoundError | InvalidTokenError> {
    return Effect.Do.pipe(
  ...
  updateDeviceProperty(
    ...
    value: unknown
  ): Effect.Effect<void, DeviceNotFoundError | DevicePropertyNotFound> {
    return Effect.Do.pipe(
      Effect.bind("device", () => this.repo.find(deviceId)),
      Effect.bind("property", ({ device }) => {
        ...
        if (property) return Effect.succeed(property)
          ...
          return Effect.fail(
            DevicePropertyNotFound(
              `Device ${device.name} does not have a property with id: ${propertyId}`
            )
          )
      }),
      ...
      Effect.tap(({ device, property }) =>
        this.propertyUpdatesSubscribers.forEach((s) =>
          s.devicePropertyChanged(device.id, property.id, value)
        )
      ),
      Effect.asVoid
    )
  ...
  updateDeviceProperties(
    deviceId: DeviceId,
    properties: Map<DevicePropertyId, unknown>
  ): Effect.Effect<void, DeviceNotFoundError | DevicePropertyNotFound> {
    return Effect.Do.pipe(
      Effect.bind("device", () => this.repo.find(deviceId)),
      Effect.bind("_", ({ device }) => {
        // Checking that all properties exist
        const notFoundProperties = Array.from(properties.keys()).flatMap((id) =>
          device.properties.find((p) => p.id === id) ? [] : [id]
        )
        if (notFoundProperties.length > 0) {
          return Effect.fail(DevicePropertyNotFound(notFoundProperties.join(", ")))
        } else {
          return Effect.void
        }
      }),
      ...
      Effect.bind("__", ({ device }) => {
        properties.forEach(
          (value, id) => (device.properties.find((p) => p.id === id)!.value = value)
        )
        return this.repo.update(device)
      }),
      Effect.mapError((e) => {
      ...
      Effect.map(({ oldProperties }) => {
        const updatedProperties = Array.from(properties).filter(
          (pair) => oldProperties.find((p) => p[0] === pair[0])![1] != pair[1]
        )
        this.propertyUpdatesSubscribers.forEach((s) => {
  ...
  setDeviceStatusUnsafe(
    deviceId: DeviceId,
    status: DeviceStatus
  ): Effect.Effect<void, DeviceNotFoundError> {
    return Effect.Do.pipe(
      Effect.bind("device", () => this.repo.find(deviceId)),
      Effect.bind("_", ({ device }) => {
  ...
  unsubscribeForDevicePropertyUpdates(subscriber: DevicePropertyUpdatesSubscriber): void {
    this.propertyUpdatesSubscribers = this.propertyUpdatesSubscribers.filter(
      (s) => s !== subscriber
    )
  }
