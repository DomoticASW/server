...
export class DeviceEventsServiceImpl implements DeviceEventsService {
  ...
  publishEvent(
    deviceId: DeviceId,
    eventName: string
  ): Effect.Effect<void, DeviceNotFoundError | NotDeviceEventError> {
    return Effect.Do.pipe(
      Effect.bind("device", () => this.devicesService.findUnsafe(deviceId)),
      Effect.bind("_", ({ device }) =>
        Effect.if(device.events.find((e) => e.name === eventName) !== undefined, {
          onTrue: () => Effect.succeed(null),
          onFalse: () => Effect.fail(NotDeviceEventError()),
        })
      ),
      Effect.tap(() =>
        this.deviceEventsSubscribers.forEach((s) =>
          s.deviceEventPublished(deviceId, DeviceEvent(eventName))
        )
      )
    )
  }
