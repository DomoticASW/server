...
class NotificationsServiceImpl implements NotificationsService {
  ...
  constructor(
    deviceStatusesService: DeviceStatusesService,
    private devicesService: DevicesService,
    private usersService: UsersService,
    private subscriptionsRepository: DeviceOfflineNotificationSubscriptionRepository
  ) {
    ...
    return pipe(
      ...
      flatMap(() =>
        this.subscriptionsRepository.add(
          DeviceOfflineNotificationSubscription(token.userEmail, deviceId)
        )
      ),
      catch_("__brand", {
  ...
  private removeSubscriptionFromRepo(email: Email, deviceId: DeviceId) {
    return this.subscriptionsRepository.remove(
      DeviceOfflineNotificationSubscription(email, deviceId)
    )
  }
  ...
  isSubscribedForDeviceOfflineNotifications(
    ...
    return pipe(
      ...
      flatMap(() => this.usersService.getUserDataUnsafe(token.userEmail)),
      flatMap(() =>
        this.subscriptionsRepository.find({ email: token.userEmail, deviceId: deviceId })
      ),
      map(() => true),
      catch_("__brand", {
        failure: "NotFoundError",
    ...
    if (status === DeviceStatus.Offline) {
      return pipe(
        ...
        flatMap((subscriptions) =>
          succeed(Array.from(subscriptions).find((s) => s.deviceId == deviceId))
        ),
  ...
  private sendNotificationToSubscriber(
    subscription: DeviceOfflineNotificationSubscription,
    deviceId: DeviceId
  ) {
    ...
    return pipe(
      ...
      match({
        onSuccess: (device) =>
          this.notificationProtocol!.sendNotification(email, `Device ${device.name} went offline.`),
        onFailure: (error) => {
...
export function NotificationsService(
  ...
  subscriptionsRepository: DeviceOfflineNotificationSubscriptionRepository
