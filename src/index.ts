...
const mongoDBConnection = await mongoose
  .createConnection(`mongodb://${mongoHost}:${mongoPort}/DomoticASW`)
  .asPromise()
...
const deviceDiscoverer = new DeviceDiscovererUDPAdapter(
  parsePortEnvVar("DISCOVERY_PORT", 30000),
  5,
  { logAnnounces: parseBooleanEnvVar("LOG_ANNOUNCES") }
)
const devicesService = new DevicesServiceImpl(
  deviceRepository,
...
const permissionsService: PermissionsService = new PermissionsServiceImpl(
  userDevicePermissionRepo,
  taskListsRepo,
  editListRepo,
  usersService,
  devicesService
...
const deviceActionsService = new DeviceActionsServiceImpl(
  devicesService,
  usersService,
  permissionsService,
  deviceCommunicationProtocol
...
const deviceStatusesService: DeviceStatusesService = new DeviceStatusesServiceImpl(
  5000,
  devicesService,
  deviceCommunicationProtocol,
  { logDeviceStatusChanges }
)

const deviceGroupRepository = new DeviceGroupRepositoryMongoAdapter(mongoDBConnection)
const deviceGroupsService = new DeviceGroupsServiceImpl(
...
const deviceOfflineNotificationSubscriptionRepository =
  new DeviceOfflineNotificationSubscriptionRepositoryMongoAdapter(mongoDBConnection)
const notificationsService = NotificationsService(
  deviceStatusesService,
  devicesService,
  usersService,
  deviceOfflineNotificationSubscriptionRepository
)

const scriptRepository = new ScriptRepositoryMongoAdapter(mongoDBConnection)
const scriptsService = new ScriptsServiceImpl(
  scriptRepository,
  devicesService,
...
const httpServer = new HTTPServerAdapter(
  ...
  scriptsService,
  permissionsService,
  { logRequestUrls, logRequestBodies, randomDelayMultiplierMs, baseDelayMs }
)
...
function parsePortEnvVar(varName: string, defaultPort: number): number {
  ...
  return Effect.Do.pipe(
    Effect.bind("portStr", () =>
      process.env[varName]
        ? Effect.succeed(process.env[varName])
        : Effect.fail<EnvVarNotSet>("EnvVarNotSet")
    ),
    ...
    Effect.bind("_", ({ portInt }) =>
      portInt >= 0 && portInt <= 65535 ? Effect.void : Effect.fail<InvalidEnvVar>("InvalidEnvVar")
    ),
    ...
    Effect.catchAll((err) => {
      switch (err) {
        ...
        case "InvalidEnvVar":
          console.log(
            `${varName}=${process.env[varName]} was not a valid port, using default: ${defaultPort}`
          )
          break
      }
